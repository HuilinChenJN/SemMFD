...
from feature_fusion.TeacherStudentModel import TeacherModel
##########################################################################
...
class CGCN(torch.nn.Module):
    def __init__(self, features, meta_label, num_user, num_item, dim_C, aggr_mode, num_routing, has_act, has_norm, is_word=False):
        ...
        if is_word:
            ...
            self.feature_extractor_model = FeatureExtractorModel(self.features, self.dim_feat,
                                                           self.meta_label, dim_C)
    ...
    def forward(self, edge_index, item_tensor):
        ...
        # 根据输入的item_tensor 计算当前小批次的loss
        item_nodes = item_tensor - self.num_user
        # print(item_tensor, item_nodes)
        teacher_feature, teacher_loss, feature_loss = self.feature_extractor_model(item_nodes)

        return x + x_hat_1, self.conv_embed_1.alpha.view(-1, 1), teacher_loss, feature_loss, teacher_feature
...
class Net(torch.nn.Module):
    def __init__(self, num_user, num_item, edge_index, user_item_dict, reg_weight, 
        ...
        if v_feat is not None:
            self.v_gcn = CGCN(self.v_feat, self.meta_data, num_user, num_item, dim_C, aggr_mode, num_routing, has_act, has_norm)
            num_model += 1

        if a_feat is not None:
            self.a_gcn = CGCN(self.a_feat, self.meta_data, num_user, num_item, dim_C, aggr_mode, num_routing, has_act, has_norm)
            num_model += 1
        
        if t_feat is not None:
            self.t_gcn = CGCN(self.t_feat, self.meta_data, num_user, num_item, dim_C, aggr_mode, num_routing, has_act, has_norm, is_word)
            num_model += 1

        self.model_specific_conf = nn.Parameter(nn.init.xavier_normal_(torch.rand((num_user+num_item, num_model))))

        self.feature_fusion_model = TeacherModel(self.meta_data, dim_C)
    ...
    def forward(self, item_tensor):
        ...
        if self.v_feat is not None:
            num_modal += 1
            v_rep, weight_v, v_teacher_loss, v_feature_loss, teacher_v = self.v_gcn(edge_index, item_tensor)
            weight = weight_v
            content_rep = v_rep
            teacher_loss = v_teacher_loss
            feature_loss = v_feature_loss

        if self.a_feat is not None:
            num_modal += 1
            a_rep, weight_a, a_teacher_loss, a_feature_loss, teacher_a = self.a_gcn(edge_index, item_tensor)
            if weight is  None:
                weight = weight_a  
                content_rep = a_rep
                teacher_loss = a_teacher_loss
                feature_loss = a_feature_loss
            else:
                teacher_loss = teacher_loss + a_teacher_loss
                feature_loss = feature_loss + a_feature_loss
        ...
        if self.t_feat is not None:
            num_modal += 1
            t_rep, weight_t, t_teacher_loss, t_feature_loss, teacher_t = self.t_gcn(edge_index, item_tensor)
            if weight is None:
                weight = weight_t   
                content_rep = t_rep
                teacher_loss = t_teacher_loss
                feature_loss = t_feature_loss
            else:
                teacher_loss = teacher_loss + t_teacher_loss
                feature_loss = feature_loss + t_feature_loss
        ...
        # 平均每个模态的损失
        teacher_loss = teacher_loss / num_modal
        ...
        fused_feature = torch.cat([teacher_v, teacher_t], dim=1)
        nodes = item_tensor - self.num_user
        fusion_loss = self.feature_fusion_model(nodes, fused_feature)

        return representation, teacher_loss, fusion_loss, feature_loss


    def loss(self, user_tensor, item_tensor):
        user_tensor = user_tensor.view(-1)
        item_tensor = item_tensor.view(-1)
        out, teacher_loss, fusion_loss, feature_loss = self.forward(item_tensor)
        ...
        return loss, bpr_loss, teacher_loss, reg_loss, extra_feature_loss
    ...
    def accuracy(self, step=2000, topk=10):
        ...
        for row, col in self.user_item_dict.items():
            ...
            for i in range(min(num_hit, topk)):
                max_ndcg_score += 1 / (np.log(i + 2) / np.log(2))
            if max_ndcg_score == 0:
                continue

            for i, temp_item in enumerate(items_list):
                if temp_item in pos_items:
                    ndcg_score += 1 / (np.log(i + 2) / np.log(2))
