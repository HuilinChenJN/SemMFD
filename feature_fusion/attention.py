...
def attention(Q, K, V, mask, dropout=None):
    ...
    if mask is not None:
        sm_input = sm_input.masked_fill(mask == 0, 1e-8)
...
class MultiheadedAttention(nn.Module):
    ...
    def __init__(self, d_model_Q, d_model_K, d_model_V, H, dout_p=0.0, d_model=None):
        ...
        # self.d_model = 2048 H=2 self.d_k =1024
    ...
    def forward(self, Q, K, V, mask):
        ...
        # (B, H, Sm, d_k) <- (B, Sm, D)
        Q = Q.view(-1, self.H, self.d_k).transpose(-3, -2)  # (-4, -3*, -2*, -1)
        K = K.view(-1, self.H, self.d_k).transpose(-3, -2)
        V = V.view(-1, self.H, self.d_k).transpose(-3, -2)
        ...
        # (B, Sq, D) <- (B, H, Sq, d_k)
        Q = Q.transpose(-3, -2).contiguous().view(Sq, self.d_model)
